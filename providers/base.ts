...
class Base {
	...
	protected get<T = unknown>(url: string, params: ObjectKeyDynamicI = {}, config: AxiosRequestConfig = {}): Promise<T> {
	...
	protected _download(url: string, params: ObjectKeyDynamicI = {}, config: AxiosRequestConfig = {}): Promise<any> {
		return new Promise((resolve, reject) => {
			const _isParam = Object.keys(params).length > 0
			this.axios
